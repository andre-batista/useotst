...
Quality First outperforms Move Fast Break Things despite 2.4$\times$ slower initial velocity: 139.89 vs 57.91 features (2.4$\times$), \$142.20 vs \$41.07 revenue (246\% higher), sustainability 0.998 vs 0.269. Move Fast accumulates debt$=$110.20 imposing 71\% revenue penalty via Loop B1, while Quality First maintains zero debt enabling sustained productivity (Figure~\ref{fig:exp4_tradeoffs}).
...
Strategic implications: capital operates as binary survival gate, PMF achievement proves capital-independent, performance remains uniform across 10$\times$ capital range, and optimal capitalization targets minimum viable runway to bridge profitability gaps. The specific 200-month threshold reflects model parameterization (Section~\ref{sec:limitations}) and should be interpreted qualitatively.
